...
Finally, create a minimal config file `tailwind.config.cjs` in the root of your project, update your content `paths` and set `darkMode` to `class`.
...
Adding a fallback initial state is the most common way to solve problem. However, it creates a new problem called _UI flick_. Essentially, the UI will flick every time the server render doesn't match your client render. 
