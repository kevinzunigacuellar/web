...
The package uses glob patterns to match the page with the layout. It then injects the layout property into the frontmatter of the page. The glob matcher uses  `picomatch` which is a fast and reliable glob matcher.
...
The `layoutOptions` object is where you define the glob pattern and the layout that you want to use. The glob pattern is the path to the page, and the layout is the path to the layout file.

You can get as specific as you want with the glob pattern. For example, you can use:
...
Even if your content is in a different folder, you can still use the package. For example, if you have your content in the `src/content` folder, you can use:
...
Then you can use the alias in the `layoutOptions`
